...
public class PaddleOcrProvider implements OcrProvider {
    ...
    public OcrResult recognize(byte[] image, String fileName, String mimeType) {
        ...
        try {
            OcrResult result = client.post()
                    .uri(runtimeUrl + "/v1/ocr")
            ...
            if (result == null) {
                throw new RestClientException("Empty OCR runtime response");
            }
            ...
            throw new ApiException(
                    ...
                    ApiErrorCode.OCR_PROVIDER_UNAVAILABLE,
    ...
    public OcrHealth health() {
        try {
            OcrHealth health = client.get()
                    .uri(runtimeUrl + "/health")
                    .retrieve()
                    .body(OcrHealth.class);

            return health == null
                    ? new OcrHealth(false, "PaddleOCR PP-OCRv5", "Réponse vide")
                    : health;

        } catch (RestClientException exception) {
            return new OcrHealth(
                    false,
                    "PaddleOCR PP-OCRv5",
